...
async function checkCoordinator() {
    if (MY_IP === coordinator || isElectionInProgress)
        return;
    return new Promise((resolve) => {
        let responded = false;
        const checkSocket = dgram_1.default.createSocket('udp4');
...
socket.on('message', (msg, rinfo) => {
    ...
    // Все серверы должны отвечать на TIME запросы, если они координаторы
    if (str === 'TIME') {
    ...
    // Все серверы должны отвечать на PING запросы (health checks)
    if (str === 'PING') {
